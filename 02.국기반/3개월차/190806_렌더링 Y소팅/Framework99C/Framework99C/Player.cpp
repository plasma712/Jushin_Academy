...
int CPlayer::Update()
{
	CObjectMgr::GetInstance()->AddLayer(RENDER_GAMEOBJECT, this);
...
{
	// 화면 상의 좌표와 비교를 해야하기 때문에
	// 현재 게임 월드 좌표 기준인 플레이어 좌표에 스크롤 값을 빼줄 것.
	if (WINCX / 2 + 200.f < m_tInfo.fX - CScrollMgr::m_fScrollX)
		CScrollMgr::m_fScrollX += m_fSpeed;
	if (WINCX / 2 - 200.f > m_tInfo.fX - CScrollMgr::m_fScrollX)
		CScrollMgr::m_fScrollX -= m_fSpeed;
	if (WINCY / 2 + 200.f < m_tInfo.fY - CScrollMgr::m_fScrollY)
		CScrollMgr::m_fScrollY += m_fSpeed;
	if (WINCY / 2 - 200.f > m_tInfo.fY - CScrollMgr::m_fScrollY)
		CScrollMgr::m_fScrollY -= m_fSpeed;
}
