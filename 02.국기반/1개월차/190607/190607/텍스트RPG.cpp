...
void Atk_Hp_Increase(tagPlayer* _tPlayer);
void LEVELUP();
...
void MonsterProduceRegen(tagMonster _tMonster[], int _iStageNumber)
{
	if (_iStageNumber == 1)
	{
		...
		*_tMonster[_iStageNumber - 1].fExp = 10.0f;
		*_tMonster[_iStageNumber - 1].iGold = 100;
	}
	else if (_iStageNumber == 2)
	{
		...
		*_tMonster[_iStageNumber - 1].fExp = 15.0f;
		*_tMonster[_iStageNumber - 1].iGold = 200;

	}
	else if (_iStageNumber == 3)
	{
		...
		*_tMonster[_iStageNumber - 1].fExp = 20.0f;
		*_tMonster[_iStageNumber - 1].iGold = 400;
	...
	*_tMonster[_StageIndex].fHp -= _tPlayer->fAtk;
	if (*_tMonster[_StageIndex].fHp <= 0)
	{
		...
		if (_tPlayer->bPlayerLife == true)
		{
			...
			_tPlayer->iGold += *_tMonster[_StageIndex].iGold;
...
void PlayerStats(tagPlayer* tPlayer)
{
	...
	cout << "|    Atk : " << int(tPlayer->fAtk) << "    		 	 |" << endl;
	cout << "|     Hp : " << int(tPlayer->fHp) << "    		 	 |" << endl;
	cout << "|    EXP : " << tPlayer->fExp << "    		 	 |" << endl;
	cout << "| 소지금 : " << tPlayer->iGold << "    		 	 |" << endl;
	cout << "==========================================" << endl;
}
